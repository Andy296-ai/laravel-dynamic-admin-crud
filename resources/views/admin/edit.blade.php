...
@section('title', 'Edit Row - ' . ucfirst($table))
    ...
    <div class="bg-white shadow rounded-lg">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500 mt-1">
                Table: <span class="font-semibold">{{ $table }}</span> | 
                ID: <span class="font-semibold">{{ $id }}</span>
            </p>
        </div>
        
        <!-- Form -->
        <form method="POST" action="{{ route('admin.update', [$table, $id]) }}" class="p-6">
            @csrf
            @method('PUT')
                ...
                @foreach($formColumns as $column)
                    <div>
                        <label for="{{ $column['name'] }}" class="block text-sm font-medium text-gray-700 mb-2">
                            ...
                            @endif
                            @if(!$column['nullable'] && $column['default'] === null && $column['name'] !== $primaryKey)
                        ...
                        @else
                            <input 
                                ...
                                value="{{ $value }}"
                                class="{{ $inputClass }} @error($column['name']) border-red-500 @enderror"
                                @if(!$column['nullable'] && $column['default'] === null && $column['name'] !== $primaryKey) required @endif
